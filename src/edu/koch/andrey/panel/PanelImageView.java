...
public class PanelImageView extends JPanel
{
	...
	private Vector<Point> polygonSelection;
	...
	public PanelImageView ()
	{
		...
		polygonSelection = new Vector<Point>();
	}
	...
	{
		if (polygonSelection == null || polygonSelection.size() == 0) { return null; }

		Polygon displayedPolygon = new Polygon();

		for (int i = 0; i < polygonSelection.size(); i++)
		{
			if (polygonSelection.get(i).x <= 0) { polygonSelection.get(i).x = 1; }
			if (polygonSelection.get(i).y <= 0) { polygonSelection.get(i).y = 1; }
			if (polygonSelection.get(i).x > imageOriginal.getWidth(this)) { polygonSelection.get(i).x = imageOriginal.getWidth(this) - 1;}
			if (polygonSelection.get(i).y > imageOriginal.getHeight(this)) { polygonSelection.get(i).y = imageOriginal.getHeight(this) - 1;}

			displayedPolygon.addPoint(polygonSelection.get(i).x, polygonSelection.get(i).y);
		}
	...
	public void displayImage ()
	{
		if (isLoaded)
		{
			...
			if (polygonSelection.size() != 0)
			{
				...
				//--- Draw poly points in green, default size - 10
				int DEF_SIZE = 10;
				for (int k = 0; k < polygonSelection.size(); k++) 
				{ 
					GraphDriver.setColor(new java.awt.Color (0, 255, 0, 100));
					GraphDriver.fillRect(polygonSelection.get(k).x - DEF_SIZE/2, polygonSelection.get(k).y - DEF_SIZE/2 , DEF_SIZE, DEF_SIZE); 
				}
				
				
				for (int k = 0; k < polygonSelection.size(); k++) { polygonToDisplay.addPoint(polygonSelection.get(k).x, polygonSelection.get(k).y); }
	...
	public void polygonDisplay (Point[] inputPolygonVertex)
	{
		polygonSelection.removeAllElements();


		for (int i = 0; i < inputPolygonVertex.length; i++)
		{
			polygonSelection.addElement(new Point(inputPolygonVertex[i].x, inputPolygonVertex[i].y));
		}

		displayImage();
	}
	
	public void polygonAddPoint (Point newPoint)
	{
		polygonSelection.addElement(newPoint);
		
		displayImage();
	}

	public void polygonRemoveAll ()
	{
		polygonSelection.removeAllElements();
	}
	...
	public boolean isInPolygon (int pixelX, int pixelY)
	{
		if (polygonSelection.size() != 0)
		{
			...
			for (int i = 0; i < polygonSelection.size(); i++) { tempPolygon.addPoint(polygonSelection.get(i).x, polygonSelection.get(i).y);}
	...
	public void transformMovePolygon (int translateX, int translateY)
	{
		for (int i = 0; i < polygonSelection.size(); i++) 
		{ 
			polygonSelection.get(i).x += translateX;
			polygonSelection.get(i).y += translateY;
		} 
